...
def conduct_CDSPM_ablation_e(repo_name, min_len, max_len):
    ...
    for train_idx, test_idx in d.split(X, Y):
        ...
        generate_input_sequence_e(x_train, y_train, data_dir, 'input_sequences_' + str(count) + '.json')
        generate_input_sequence_e(x_test, y_test, data_dir, 'test_sequences_' + str(count) + '.json')
        # generate_all_sequence_ete(D, data_dir, 'all_sequences_symbol_ver.json', use_entropy=True)

        CDSPM(data_dir, count)
        test, pred, temp_list = validate_seq_vector(data_dir, count, use_PCA=False)
        total_test += test
        for i in pred:
            total_pred.append(i)
...
def conduct_CDSPM_ablation_ee(repo_name, min_len, max_len):
    cut_sequence(min_len, max_len, repo_name, interval=3)
    X, Y, D = generate_dataset(repo_name)
    data_dir = get_global_val('result_dir') + repo_name + '_' + str(min_len) + '_' + str(max_len) + '_ee'
    if not os.path.exists(data_dir):
        os.mkdir(data_dir)
    write_json_list(D, os.path.join(data_dir, 'all_sequences.json'))
    ...
    for train_idx, test_idx in d.split(X, Y):
        ...
        generate_input_sequence_ee(x_train, y_train, data_dir, 'input_sequences_' + str(count) + '.json')
        generate_input_sequence_ee(x_test, y_test, data_dir, 'test_sequences_' + str(count) + '.json')
        # generate_all_sequence_ete(D, data_dir, 'all_sequences_symbol_ver.json', use_entropy=True)
...
if __name__ == '__main__':
    ...
    # recommend_actions('tensorflow_9_30', model_idx=8)
    # translate_result(repo)
